(() => {
  ...
  modalConfirmBtn?.addEventListener('click', () => hideModal(modalInput?.value || ''));
  modalCancelBtn?.addEventListener('click', () => hideModal(null));
  modalOverlay?.addEventListener('click', () => hideModal(null));
  ...
  /* -------------------- mock data store -------------------- */
  const MEETINGS_KEY = 'vc_standalone_meetings';
  const USERS_KEY = 'vc_standalone_users';
  ...
  /* -------------------- mock courses -------------------- */
  const mockCourses = [
    { id: 1, title: 'Dasar Kecerdasan Artifisial', icon: '🧠', instructor: 'Bambang Subeno, S.T., M.Kom.', participants: 24, duration: '150 menit', room: 'Lab A-201', status: 'upcoming', schedule: 'Kelas IF-48-11 • Senin 07:00' },
    { id: 2, title: 'Interaksi Manusia Komputer', icon: '🖥️', instructor: 'Iwan Abadi, S.Kom., MM., MT., CSCU., CEI', participants: 22, duration: '90 menit', room: 'Ruang 203', status: 'upcoming', schedule: 'Kelas IF-48-10 • Rabu 10:30' },
    { id: 3, title: 'Jaringan Komputer', icon: '🌐', instructor: 'Ghifari Ramadhika Permana, S.Kom., M.Sc.', participants: 20, duration: '200 menit', room: 'Lab A-201', status: 'live', schedule: 'Kelas IF-48-10 • Rabu 07:00' },
    { id: 4, title: 'Pemrograman Berorientasi Objek', icon: '💻', instructor: 'Soni Yora, S.Kom., M.Kom.', participants: 20, duration: '200 menit', room: 'Lab B-101', status: 'upcoming', schedule: 'Kelas IF-48-10 • Selasa 07:00' },
    { id: 5, title: 'Strategi Algoritma', icon: '📐', instructor: 'Fauzan Firdaus, S.Kom., M.T.', participants: 18, duration: '150 menit', room: 'Ruang 105', status: 'idle', schedule: 'Kelas IF-48-04 • Kamis 07:00' },
    { id: 6, title: 'Wawasan Global TIK', icon: '🌍', instructor: 'Ahmad Bintang Arif, S.Kom., M.Kom. & Donni Richasdy, S.T., M.T.', participants: 16, duration: '100 menit', room: 'Ruang 105', status: 'upcoming', schedule: 'Kelas IF-48-10 • Senin 09:40' },
  ];
  ...
  function handleLogin(email, password) {
    ...
    if (!user) {
      if (email === 'akbar@example.com' && password === 'password123') {
        const token = btoa(JSON.stringify({ id: 1, name: 'Akbar Saputra', email: 'akbar@example.com' }));
        authToken = token;
        authUser = { id: 1, name: 'Akbar Saputra', email: 'akbar@example.com' };
  ...
  function createMeeting(title) {
    ...
    const meeting = { id: Date.now(), code: generateCode(), title, host_id: authUser?.id || 1, host_name: authUser?.name || 'Pengguna', status: 'active', created_at: new Date().toISOString() };
  ...
  function getMeetingByCode(code) {
    const meetings = getStoredMeetings();
    const meeting = meetings.find(m => m.code === code && m.status === 'active');
    if (!meeting) throw new Error('Meeting tidak ditemukan atau sudah berakhir');
    return meeting;
  }
  ...
  function endMeetingApi(id) {
    const meetings = getStoredMeetings();
    const idx = meetings.findIndex(m => m.id === id);
    if (idx !== -1) { meetings[idx].status = 'ended'; saveMeetings(meetings); }
  }
  ...
  function renderCourses(courses) {
    ...
    courses.forEach(c => {
      ...
      if (c.status === 'live') {
        ...
        badgeHtml = `<span class="badge badge-soon">${sub.replace('Kelas ', '')}</span>`;
        btnHtml = `<button class="btn ghost" data-action="goto" data-target="detail" data-i18n="view_detail">Lihat detail</button>`;
      }
  ...
  function stopMic() {
    if (streams.mic) { streams.mic.getTracks().forEach(t => t.stop()); streams.mic = null; }
  ...
  function stopCam() {
    if (streams.cam) { streams.cam.getTracks().forEach(t => t.stop()); streams.cam = null; }
  ...
  function syncPreview() {
    ...
    if (previewTag) previewTag.textContent = state.cam ? t("preview_tag_cam_active") : t("preview_tag_cam_off");
  }
  ...
  toggles.forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (key === 'mic') { if (state.mic) await startMic(); else stopMic(); }
      else if (key === 'cam') { if (state.cam) await startCam(); else stopCam(); }
  ...
  const prejoinObserver = new MutationObserver(() => {
    if (prejoinEl?.classList.contains('active')) { prejoinMediaStarted = false; startPrejoinMedia(); }
  });
  ...
  function renderNotifications() {
    ...
    if (query) filtered = filtered.filter(n => n.title.toLowerCase().includes(query) || n.desc.toLowerCase().includes(query));
    filtered.sort((a, b) => {
    ...
    if (filtered.length === 0) { notifList.innerHTML = '<p class="notif-empty">Tidak ada notifikasi.</p>'; return; }
    filtered.forEach(n => {
      ...
      item.innerHTML = `
        ...
        <div class="notif-item-body">
          <div class="notif-item-title">${n.title}<span class="notif-item-priority ${n.priority}">${getNotifPriorityLabel(n.priority)}</span></div>
          <p class="notif-item-desc">${n.desc}</p>
          <div class="notif-item-time">${timeAgo(n.time)}<span class="notif-item-role">${n.role === 'all' ? t('notif_role_all') : n.role === 'dosen' ? t('notif_role_lecturer') : t('notif_role_student')}</span></div>
        </div>`;
      item.addEventListener('click', (e) => { e.stopPropagation(); openNotifDetail(n); });
  ...
  function updateNotifBadge() {
    ...
    if (unread > 0) {
      if (existing) existing.textContent = unread > 9 ? '9+' : unread;
      else if (notifFab) {
    ...
    } else { if (existing) existing.remove(); notifFab?.classList.remove('notif-fab'); }
  }
  ...
  function openNotifPanel() {
    ...
    renderNotifications();
    if (notifSearchInput) { notifSearchInput.value = ''; notifSearchInput.focus(); }
  }

  function closeNotifPanel() { notifDropdown?.classList.remove('open'); }
  ...
  function openNotifDetail(n) {
    ...
    if (!n.read) { n.read = true; renderNotifications(); updateNotifBadge(); }
    ...
    if (n.attachment && n.attachment.name) { attachSection.hidden = false; document.getElementById('notifDetailAttachName').textContent = n.attachment.name; }
    else { attachSection.hidden = true; }
    notifDetail?.classList.add('open');
  }

  function closeNotifDetail() { notifDetail?.classList.remove('open'); currentNotifDetail = null; }
  ...
  document.getElementById('notifDetailReadBtn')?.addEventListener('click', () => {
    if (currentNotifDetail) { currentNotifDetail.read = true; renderNotifications(); updateNotifBadge(); toast('Notifikasi ditandai sudah dibaca'); closeNotifDetail(); }
  });
  document.getElementById('notifDetailAttachBtn')?.addEventListener('click', () => {
    if (currentNotifDetail?.attachment?.url) toast('Membuka: ' + currentNotifDetail.attachment.name);
  });
  notifFab?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (notifDropdown?.classList.contains('open')) closeNotifPanel();
    else openNotifPanel();
  });
  ...
  notifLangFilter?.addEventListener('change', () => {
    ...
    if (globalSelect) { globalSelect.value = lang; globalSelect.dispatchEvent(new Event('change')); }
  });
  notifSearchInput?.addEventListener('input', renderNotifications);
  document.addEventListener('click', (e) => {
    if (notifDropdown?.classList.contains('open') && !notifDropdown.contains(e.target) && e.target !== notifFab) closeNotifPanel();
  });
  updateNotifBadge();
  ...
  function renderSchedule() {
    ...
    list.innerHTML = '';
    if (filtered.length === 0) { list.innerHTML = '<p class="schedule-empty" data-i18n="no_schedule">' + t('no_schedule') + '</p>'; return; }
    filtered.forEach(s => {
  ...
  function initSchedule() {
    ...
    if (semesterSelect) { semesterSelect.value = selectedSemester; semesterSelect.addEventListener('change', () => { selectedSemester = semesterSelect.value; renderSchedule(); }); }
    if (prodiSelect) { prodiSelect.value = selectedProdi; prodiSelect.addEventListener('change', () => { selectedProdi = prodiSelect.value; renderSchedule(); }); }
    document.querySelectorAll('.schedule-day-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        document.querySelectorAll('.schedule-day-chip').forEach(c => c.classList.remove('active'));
        chip.classList.add('active');
  ...
  function renderGrades() {
    ...
    if (!list) return;
    const displaySemesters = selectedGradeSemester === 'all' ? gradesData.semesters : gradesData.semesters.filter(s => s.name === selectedGradeSemester);
    ...
    list.innerHTML = '';
    if (displaySemesters.length === 0) { list.innerHTML = '<p class="grades-empty">Tidak ada data nilai</p>'; return; }
    displaySemesters.forEach(sem => {
      sem.courses.forEach(c => {
        ...
        card.innerHTML = `
          <div class="grades-course-header"><span class="grades-course-name">${c.name}</span><span class="grades-course-sks">${c.sks} SKS</span></div>
          <div class="grades-components">${compHtml}</div>
          <div class="grades-footer"><span class="grades-grade-badge ${gradeColorClass(c.grade)}">${c.grade}</span><span class="grades-status ${c.passed ? 'grades-status-passed' : 'grades-status-failed'}">${c.passed ? 'Lulus' : 'Tidak Lulus'}</span></div>`;
        list.appendChild(card);
      });
    });
  }
  ...
  document.addEventListener('click', (e) => {
    if (profileDropdown?.classList.contains('open') && !profileDropdown.contains(e.target) && e.target !== avatarBtn) profileDropdown.classList.remove('open');
  });

  $('#logoutBtn')?.addEventListener('click', () => { profileDropdown?.classList.remove('open'); logout(); });
  document.querySelector('[data-action="goto-profile"]')?.addEventListener('click', () => { profileDropdown?.classList.remove('open'); });
  ...
  /* -------------------- Meeting Room (standalone, local video only) -------------------- */
  ...
  async function startLocalMedia() {
    try {
      ...
      if (video) { video.srcObject = localStream; video.classList.add('active'); }
  ...
  function enterMeetingRoom(meeting) {
  ...
  document.getElementById('meetCamBtn')?.addEventListener('click', () => {
    ...
    const video = document.getElementById('localVideo');
    const avatar = document.getElementById('localAvatar');
    if (video && avatar) { video.classList.toggle('active', enabled); avatar.classList.toggle('hidden', enabled); }
  });
  ...
  function loadActiveMeetings() {
    ...
    setTimeout(() => {
      ...
      if (!container) return;
      container.innerHTML = '';
      const meetings = listActiveMeetings();
      if (!meetings || meetings.length === 0) { if (empty) empty.hidden = false; return; }
      if (empty) empty.hidden = true;
      meetings.forEach(m => {
        const item = document.createElement('div');
        item.className = 'active-meeting-item';
        item.innerHTML = `<div class="active-meeting-icon">📹</div><div class="active-meeting-info"><h4>${m.title}</h4><p>oleh ${m.host_name}</p></div><div class="active-meeting-code">${m.code}</div>`;
        item.addEventListener('click', () => { try { enterMeetingRoom(m); } catch (err) { toast(err.message); } });
  ...
  const lobbyObserver = new MutationObserver(() => { if (lobbyEl?.classList.contains('active')) loadActiveMeetings(); });
  ...
  document.getElementById('meetFab')?.addEventListener('click', () => {
    if (!isLoggedIn()) { toast('Silakan masuk terlebih dahulu'); return; }
    show('meetingLobby');
  });

  document.getElementById('createMeetingBtn')?.addEventListener('click', async () => {
    const title = await showModal({ title: t('create_meeting_title'), placeholder: t('create_meeting_placeholder'), confirmText: t('create_meeting_confirm'), cancelText: t('create_meeting_cancel') });
    if (!title || !title.trim()) return;
    try {
      const meeting = createMeeting(title.trim());
  ...
  document.getElementById('meetLeaveBtn')?.addEventListener('click', () => {
    if (currentMeetingId) endMeetingApi(currentMeetingId);
    leaveMeeting();
  });
  ...
  document.getElementById('meetShareBtn')?.addEventListener('click', () => {
    ...
    if (!code) return;
    navigator.clipboard.writeText(code).then(() => toast('Kode meeting disalin: ' + code)).catch(() => toast('Kode meeting: ' + code));
  });
  ...
  function getAbsensiData() { try { return JSON.parse(localStorage.getItem(ABSENSI_KEY)) || []; } catch { return []; } }
  function saveAbsensiData(data) { localStorage.setItem(ABSENSI_KEY, JSON.stringify(data)); }
  function formatTime(d) { return d.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }); }
  function formatDate(d) { return d.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }); }
  ...
  function getTodayRecords(data) { const today = new Date(); return data.filter(r => new Date(r.date).toDateString() === today.toDateString()); }
  function getActiveSession() { return getTodayRecords(getAbsensiData()).find(r => !r.checkout) || null; }

  function calcHours(records) {
    return records.reduce((total, r) => { const start = new Date(r.checkin); const end = r.checkout ? new Date(r.checkout) : new Date(); return total + (end.getTime() - start.getTime()); }, 0);
  }
  ...
  function getMonthRecords(data) {
    const now = new Date();
    return data.filter(r => { const d = new Date(r.date); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); });
  }
  ...
  function updateAbsensiDisplay() {
    ...
    if (session) {
      ...
      if (quickStatus) { quickStatus.textContent = (t('checked_in') || 'Sedang check-in') + ' ' + formatTime(new Date(session.checkin)); quickStatus.style.color = 'var(--success)'; }
      checkinBtn?.setAttribute('disabled', 'true');
      checkoutBtn?.removeAttribute('disabled');
    } else {
      if (statusIcon) statusIcon.textContent = '⏳';
      if (statusText) statusText.textContent = t('not_checked_in') || 'Belum check-in';
      if (quickStatus) { quickStatus.textContent = t('not_checked_in') || 'Belum check-in'; quickStatus.style.color = ''; }
  ...
  function updateAbsensiStats() {
    ...
    document.getElementById('absensiTodayHours').textContent = formatHours(calcHours(getTodayRecords(data)));
    document.getElementById('absensiWeekHours').textContent = formatHours(calcHours(getWeekRecords(data)));
    document.getElementById('absensiMonthHours').textContent = formatHours(calcHours(getMonthRecords(data)));
  }
  ...
  function renderAbsensiHistory() {
    ...
    list.innerHTML = '';
    if (sorted.length === 0) { if (empty) empty.hidden = false; return; }
    if (empty) empty.hidden = true;
    sorted.slice(0, 20).forEach(r => {
      ...
      item.innerHTML = `<div class="absensi-history-icon">${end ? '✅' : '🟢'}</div><div class="absensi-history-body"><div class="absensi-history-date">${formatDate(start)}</div><div class="absensi-history-detail">${formatTime(start)}${end ? ' — ' + formatTime(end) : ' — ...'}</div></div><div class="absensi-history-hours">${hours}</div>`;
  ...
  function absensiCheckout() {
    ...
    if (record) record.checkout = new Date().toISOString();
    saveAbsensiData(data);
    updateAbsensiDisplay();
    updateAbsensiStats();
    renderAbsensiHistory();
    toast((t('check_out_success') || 'Check-out berhasil. Total:') + ' ' + formatHours(Date.now() - new Date(session.checkin).getTime()));
  }

  function absensiExportReport() {
    const data = getAbsensiData();
    if (data.length === 0) { toast('Belum ada data absensi untuk diexport.'); return; }
    const rows = [['Tanggal', 'Check In', 'Check Out', 'Durasi']];
    data.forEach(r => {
      const start = new Date(r.checkin);
      const end = r.checkout ? new Date(r.checkout) : null;
      rows.push([formatDate(start), formatTime(start), end ? formatTime(end) : '-', end ? formatHours(end.getTime() - start.getTime()) : 'Berlangsung']);
    });
  ...
  function stopAbsensiTimer() {
    ...
    if (absensiTimer) { clearInterval(absensiTimer); absensiTimer = null; }
  }
  ...
  const absensiObserver = new MutationObserver(() => {
    ...
    if (el?.classList.contains('active')) {
      ...
      renderAbsensiHistory();
      const session = getActiveSession();
      if (session) { startAbsensiTimer(); updateAbsensiTimer(); }
      else { stopAbsensiTimer(); updateAbsensiTimer(); }
    } else stopAbsensiTimer();
  });

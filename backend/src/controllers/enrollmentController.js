...
async function enroll(req, res, next) {
  try {
    ...
    const existing = await db.prepare('SELECT id FROM enrollments WHERE user_id = ? AND course_id = ?')
      .get(req.user.id, req.params.courseId);
    if (existing) return res.status(409).json({ error: 'Sudah terdaftar di kelas ini' });

    const result = await db.prepare('INSERT INTO enrollments (user_id, course_id) VALUES (?, ?)')
      .run(req.user.id, req.params.courseId);
    await db.prepare('UPDATE courses SET participants = participants + 1 WHERE id = ?').run(req.params.courseId);
...
async function unenroll(req, res, next) {
  try {
    const db = getDb();
    const enrollment = await db.prepare('SELECT * FROM enrollments WHERE id = ? AND user_id = ?')
    ...
    await db.prepare('UPDATE courses SET participants = GREATEST(0, participants - 1) WHERE id = ?').run(enrollment.course_id);
